...
const MANIFEST = {
  ...
  endpoints: [
    ...
    { method: "GET", path: "/au-company/name-availability?name={name}", price: "0.001", description: "Live ASIC company name availability check (identical names, objections)." },
    { method: "GET", path: "/au-company/live/acn/{acn}", price: "0.05", description: "Live ACN validation and lookup via Business API DSP (requires upstream account funding)." },
    { method: "GET", path: "/au-company-report?acn={acn}", price: "12.00", description: "Official ASIC company extract ordered via Business API (directors, office, share capital). Always paid." },
